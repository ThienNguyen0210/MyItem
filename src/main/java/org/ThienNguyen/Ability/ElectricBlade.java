...
public class ElectricBlade implements IAbility {
    ...
    public void execute(Player attacker, LivingEntity target, int level, double baseDamage) {
        ...
        // Tránh không cho sét đánh chính người gây ra kỹ năng ngay mục tiêu đầu tiên
        ...
        // Đưa attacker vào danh sách đã trúng để chắc chắn không bao giờ bị sét đánh ngược lại
        struckEntities.add(attacker.getEntityId());
    ...
    private void chainLightning(Player attacker, Location sourceLoc, LivingEntity currentTarget, double damage, int remainingJumps, Set<Integer> struckEntities) {
        if (currentTarget == null || remainingJumps < 0 || currentTarget.equals(attacker)) return;
        ...
        if (remainingJumps > 0) {
            // Truyền thêm attacker vào hàm tìm kiếm mục tiêu tiếp theo để lọc bỏ
            LivingEntity nextTarget = findNextChainTarget(currentTarget, struckEntities, attacker, 8.0);
            if (nextTarget != null) {
    ...
    private LivingEntity findNextChainTarget(LivingEntity current, Set<Integer> struckEntities, Player attacker, double radius) {
        List<Entity> nearby = current.getNearbyEntities(radius, radius, radius);
        LivingEntity closest = null;
        double closestDist = Double.MAX_VALUE;

        for (Entity e : nearby) {
            // Thêm điều kiện !e.equals(attacker) để loại trừ tuyệt đối người chơi kích hoạt kỹ năng
            if (e instanceof LivingEntity next && !e.equals(attacker) && !(e instanceof ArmorStand) && !struckEntities.contains(e.getEntityId())) {
